...
MyGraphicsView::MyGraphicsView(QListView *listView, QWidget *parent)// Конструктор класса MyGraphicsView
    ...
    if (globalListView && globalListView->model()) {
        connect(globalListView->selectionModel(), &QItemSelectionModel::currentChanged,
                    this, &MyGraphicsView::onListViewItemSelected);
        } else {
            qDebug( "Warning. GlobalListView is null" );
        }
}
...
//            //Добавление данных о координатах сразу при добавлении в QGraphicsView
//            QStandardItemModel* tableModel = qobject_cast<QStandardItemModel*>(tableView->model());
//                   QStandardItem* xItem = new QStandardItem(QString::number(10));
//                   QStandardItem* yItem = new QStandardItem(QString::number(10));
//                   tableModel->setItem(0, 1, xItem);
//                   tableModel->setItem(1, 1, yItem);
//           qDebug() << "Добавление координат:" << text << dropPosition.x() << dropPosition.y();
